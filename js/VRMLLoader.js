...
THREE.VRMLLoader.prototype = {
    ...
    parse : function(data) {
        ...
        var parseV2 = function(lines, scene) {
             ...
             * specified colors at the specified angles. This is used for the Background
            ...
            var paintFaces = function(geometry, radius, angles, colors, directionIsDown) {
                // console.log(geometry, radius, angles, colors, directionIsDown);
            ...
            var parseProperty = function(node, line) {
                ...
                if (this.isRecordingFaces) {
                    ...
                    switch (fieldName) {
                        ...
                        case 'color':
                            if (parts.length != 4) {
                                parts[1] = this.colors[0].r;
                                parts[2] = this.colors[0].g;
                                parts[3] = this.colors[0].b;
                                // parts[2] = .1;
                                // parts[3] = .5;
                                ...
                                // if (!window['logcount'])
                                // window['logcount'] = 0
                                // if (!window['alreadylogged' + (this.colorType || 'color') + window['logcount']]){
                                // window['mycount']=0;
                                // window['alreadylogged' + (this.colorType || 'color') + window['logcount']]={}
                                // }
                                // window['alreadylogged' + (this.colorType || 'color') + window['logcount']][window['mycount']+=1 + this.colorType] = property;
                                // window['logcount'] += 1;
                            ...
                            property = {
                                ...
                                'g' : parseFloat(parts[2]),
                                'b' : parseFloat(parts[3])
                                // 'r'         : parseFloat(parts[1] * ( !this.colorType ? 255 : 1) ),
                                // 'g'         : parseFloat(parts[2] * ( !this.colorType ? 255 : 1) ),
                                // 'b'         : parseFloat(parts[3] * ( !this.colorType ? 255 : 1) )
                            }
            ...
            var parseNode = function(data, parent) {
                ...
                if ('Transform' === data.nodeType || 'Group' === data.nodeType) {
                    ...
                    // console.log(object);
                    if(object.parent==parent){console.log(object)}
                    parent.add(object);
                    ...
                    // skyMaterial.color = new THREE.Color(0xff0000);
                    // paintFaces(skyGeometry, radius, data.skyAngle, skyMaterial.color, true);
                    ...
                    if ('Box' === data.nodeType) {
                        ...
                        for (var i = 0, j = data.children.length; i < j; i++) {
                            ...
                            if ('Coordinate' === child.nodeType) {
                                for (var k = 1, l = child.children.length; k < l; k++) {
                                // for (var k = 0, l = child.points.length; k < l; k++) {
                                    // var point = child.points[k];
                                    var point = child.children[k];
                                    var pointArr = point.replace(',','').split(' ');
                                    pointArr.length === 3 ? point = {x : pointArr[0], y : pointArr[1], z : pointArr[2]} : null;
                        ...
                        for (var i = 0, j = data.coordIndex.length; i < j; i++) {
                            ...
                            while (indexes.length >= 3 && skip < (indexes.length - 2 )) {
                                ...
                                var face = new THREE.Face3(indexes[0], indexes[skip + 1], indexes[skip + 2], 
                                    null, // normal, will be added later
                                    new THREE.Color( 0xffaa00 )// todo: pass in the color, if a color index is present
                                );
                                skip++;
                                !window['facecount'] ? window['facecount'] = 0 : window['facecount'] += 1;
                                window['facecount']<30 ? (
                                    // console.log(face.color.getHexString()),
                                    window['face']=face ):
                                    null;
                    ...
                    for (var i = 0; i < data.children.length; i++) {
                        ...
                        if ('Material' === child.nodeType) {
                            ...
                            material.opacity = .1;
                            material.castShadow = material.receiveShadow = true;
